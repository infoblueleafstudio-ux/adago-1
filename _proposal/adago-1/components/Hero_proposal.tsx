 ...
 * 
 * 【既存コード保護】
 * - 現在のcomponents/Hero.tsxは変更せず、この提案は参考用
 * - 既存のuseImageHeadline設定は保持
 * - 既存のアニメーション設定は維持
 */
...
export default function HeroProposal({ useImageHeadline = false }: HeroProps) {
  return (
    <section 
 ...
 *    - 既存の useImageHeadline 設定を保持
 * 
 * 2. アニメーションの段階化
 *    - タイトル → サブタイトル → CTA の順序
 *    - 既存の motion 設定を保持
 *    - 既存の hero-title クラスを維持
 * 
 * 3. レスポンシブ対応
 ...
 * 
 * 【既存コードとの互換性】
 * - 既存のHeroPropsインターフェースを維持
 * - 既存のuseImageHeadline設定を保持
 * - 既存のTailwindクラスを維持
 * - 既存のアニメーション設定を保持
 */
